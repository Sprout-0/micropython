...
def music_beep():
    ...
    for tone in melody:
        ...
        time.sleep_ms(50)
       '''
def beep_control(kai_qi_pin_lv,kai_qi_zhan_kong_bi,shi_jian,guan_bi_pin_lv,guan_bi_zhan_kong_bi):
    pwm.freq(kai_qi_pin_lv)  #频率，单位Hz
    pwm.duty_u16(kai_qi_zhan_kong_bi)
    time.sleep_ms(shi_jian)
    pwm.freq(guan_bi_pin_lv)  #频率，单位Hz
    pwm.duty_u16(guan_bi_zhan_kong_bi)
    time.sleep_ms(shi_jian)
def beep_on():
...
while 1:
    beep_control(4000,32768,500,4000,0)
